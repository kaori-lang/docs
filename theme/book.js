	...
	if (playgrounds.length > 0) {
		fetch_with_timeout("https://play.rust-lang.org/meta/crates", {
			...
			.then((response) => {
				...
				const playground_crates = response.crates.map(
					(item) => item["id"]
				);
...
aria-label="Show hidden lines"></button>';
		buttons.firstChild.innerHTML =
			document.getElementById("fa-eye").innerHTML;
		...
		buttons.firstChild.addEventListener("click", function (e) {
			if (this.title === "Show hidden lines") {
				this.innerHTML =
					document.getElementById("fa-eye-slash").innerHTML;
...
(function themes() {
	...
	function updateThemeSelected() {
		...
		let element = themePopup.querySelector(
			"button#mdbook-theme-" + selected
		);
		if (element === null) {
			// Fall back in case there is no "Default" item.
			element = themePopup.querySelector(
...
(function chapterNavigation() {
	document.addEventListener("keydown", function (e) {
		...
		function hideHelp() {
			document.getElementById("mdbook-help-container").style.display =
				"none";
		}
